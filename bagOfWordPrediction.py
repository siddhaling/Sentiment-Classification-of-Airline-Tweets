#Program to read tweets into data frame then perform the preprocessing
#The feature extraction is done using bag of words. The data set is split into train and test sets
#The logistic regressor is used to fit on the training data and prediction of sentiment is carried out on test data.
import re
import pandas as pd 
import numpy as np 
import warnings 
...
bagOfWords_vec=CountVectorizer(max_df=0.9, max_features=1000, stop_words='english')
bagOfWords=bagOfWords_vec.fit_transform(data['cleanStemmedT'])


xtrain_bag, xtest_bag, ytrain_bag, ytest_bag = train_test_split(bagOfWords, data['sentiLabel'], random_state=91, test_size=0.3)
